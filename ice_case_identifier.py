...
class IceLossDetector(pd.DataFrame):
    ...
    _metadata = ["temperatureCorrectionApplied","parameters","statistics"]
    ...
    def addReferencePowerToData(self, pc):
        ...
        y10 = pc[pc_mask][('output_power_low_quantile')].to_numpy()
        w = pc[pc_mask][('wind_speed_mean')].to_numpy()
        y = pc[pc_mask][('output_power_mean')].to_numpy()
        self['low_quantile_ref'] = np.interp(self['wind_speed'].to_numpy(),w,y10)
        self['reference_power'] = np.interp(self['wind_speed'].to_numpy(),w,y)
    ...
    def plotPowerCurve(self,pc):
        pc.plot(x='wind_speed_mean', y=['output_power_mean','output_power_low_quantile','output_power_high_quantile'])
        ax2 = plt.gca()
    ...
    def make_statistics(self):
        ...
        reduced_prod_events = self[self['ice_alarm'] == 1]
        stops_events = self[self['stops'] == 1]
